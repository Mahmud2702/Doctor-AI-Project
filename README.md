...
ClinicAssist AI routes patient questions to the right specialist agent — document analysis, clinic information, live medical search, or general health guidance — and responds in whichever language the user asked in.
...
- **Multi-agent routing** — a supervisor agent classifies each query and hands it off to the right specialist:
  ...
  | 📚 Clinic Information (`rag`) | Clinic hours, doctor schedules, test prices, services |
...
`app.py` never contains raw HTML/CSS — all presentation logic lives in `ui/`, so the chatbot logic and the interface can be changed independently.
...
|---|---|---|
| `GEMINI_API_KEY` | ✅ | Powers all four agents (Gemini 2.5 Flash) |
